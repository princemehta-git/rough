  ...
  const promise = logs.map(async (log, index) => {
    ...
    try {
      ...
      if (!session) {
        ...
        // console.log(`[${index}] 6 - Session found`);
        // check if number is available
        const check = await timeoutPromise(isExists(session, jid, false), 5000);
